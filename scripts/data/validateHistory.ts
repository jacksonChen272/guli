...
const symbols = manifest.items.filter((item) => item.isOfficial && item.path).map((item) => {
  ...
  return { symbol: item.symbol, name: item.name, path: item.path, firstTradeDate: item.firstDate, lastTradeDate: item.lastDate, recordCount: item.recordCount, status: dataset.status, stale: dataset.status === 'stale', warnings: dataset.warnings, technical: { aboveMa20: typeof row?.aboveMa20 === 'boolean' ? row.aboveMa20 : null, rsiOverbought: typeof row?.rsi14 === 'number' ? row.rsi14 >= 70 : null, macdGoldenCross: typeof row?.macdCrossDays === 'number', volumeExpansion: typeof row?.volumeRatio === 'number' ? row.volumeRatio >= 1.5 : null, indicatorComputable: item.eligibleForTechnical } }
...
const report = { schemaVersion: 'history-validation-summary-v1', generatedAt: manifest.updatedAt, totalFiles: datasets.size + failures.length, validFiles: datasets.size, invalidFiles: failures.length, normalizedRowsThisRun: normalizedRows, normalizedRowsTotal, commonStocks: manifest.summary.commonStocks, complete300: manifest.summary.complete, partial: manifest.summary.partial, pending: manifest.summary.pending, unsupported: manifest.summary.unsupported, technicalEligible: manifest.summary.technicalEligible, errors: failures }
...
await atomicWriteJson(path.join(root, 'data', 'history', 'history-progress.json'), buildProgress(manifest, priorProgress?.startedAt ?? manifest.updatedAt, priorProgress?.lastProcessedSymbol ?? null, priorProgress?.status ?? 'idle'))
