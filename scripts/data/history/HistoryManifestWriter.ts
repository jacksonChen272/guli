...
export function buildHistoryManifest(
  ...
  const items: HistoryManifestItem[] = securities.map((security) => {
    ...
    return { symbol: security.symbol, name: dataset.name || security.name, status, recordCount: dataset.recordCount, firstDate: dataset.firstTradeDate, lastDate: dataset.lastTradeDate, lastUpdatedAt: dataset.fetchedAt, source: 'TWSE', isOfficial: validation.valid, validationStatus: validation.valid ? 'valid' : 'invalid', errors: validation.errors, securityType: security.instrumentType, eligibleForTechnical: true, technicalDataReady: validation.valid && dataset.recordCount >= technicalMinimumDays, path: validation.valid ? `data/twse-stock-history/stocks/${security.symbol}.json` : null }
  }).sort((left, right) => left.symbol.localeCompare(right.symbol))
  const count = (status: HistoryManifestItem['status']) => items.filter((item) => item.status === status).length
  const eligibleCommonStocks = items.filter((item) => item.eligibleForTechnical).length
