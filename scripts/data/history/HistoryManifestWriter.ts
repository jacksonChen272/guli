...
async function renameWithTransientLockRetry(source: string, destination: string) {
  const delays = [0, 50, 150, 400, 1_000]
...
export async function atomicWriteJson(file: string, value: unknown) {
  ...
  try { JSON.parse(await readFile(temporary, 'utf8')); await renameWithTransientLockRetry(temporary, file) }
...
export function buildHistoryManifest(
  ...
  const items: HistoryManifestItem[] = securities.map((security) => {
    ...
    return { symbol: security.symbol, name: dataset.name || security.name, status, recordCount: dataset.recordCount, firstDate: dataset.firstTradeDate, lastDate: dataset.lastTradeDate, lastUpdatedAt: dataset.fetchedAt, source: 'TWSE', isOfficial: validation.valid, validationStatus: validation.valid ? 'valid' : 'invalid', errors: validation.errors, securityType: 'common_stock', eligibleForTechnical: true, technicalDataReady: validation.valid && dataset.recordCount >= technicalMinimumDays, path: validation.valid ? `data/twse-stock-history/stocks/${security.symbol}.json` : null }
  }).sort((left, right) => left.symbol.localeCompare(right.symbol))
  const count = (status: HistoryManifestItem['status']) => items.filter((item) => item.status === status).length
  const eligibleCommonStocks = items.filter((item) => item.eligibleForTechnical && item.securityType === 'common_stock').length
  const unsupportedSecurities = count('unsupported')
  const complete = count('complete'); const partial = count('partial'); const failed = count('failed'); const pending = count('pending')
  return { schemaVersion: 'history-manifest-v1', updatedAt, targetTradingDays, technicalMinimumDays, source: 'TWSE', storageRoot: 'data/twse-stock-history/stocks', summary: { totalSecurities: items.length, eligibleCommonStocks, unsupportedSecurities, total: items.length, commonStocks: eligibleCommonStocks, complete, partial, failed, pending, unsupported: unsupportedSecurities, officialValid: items.filter((item) => item.isOfficial).length, technicalEligible: eligibleCommonStocks, technicalReady: items.filter((item) => item.technicalDataReady).length, coverageInvariantValid: complete + partial + pending + failed === eligibleCommonStocks }, items }
}
...
export function buildProgress(manifest: HistoryManifest, startedAt: string, lastProcessedSymbol: string | null, status: HistoryProgress['status']): HistoryProgress {
  const common = manifest.items.filter((item) => item.securityType === 'common_stock' && item.eligibleForTechnical)
