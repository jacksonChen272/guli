...
export function GlobalStockSearch({ variant = 'topbar' }: { variant?: 'topbar' | 'hero' } = {}) {
  ...
  const [recent, setRecent] = useState<string[]>(() => repositoryHub.recentSearch.getRecent(10))
  ...
  const select = (symbol: string) => {
    setRecent(repositoryHub.recentSearch.record(symbol))
    close()
    navigate(`/stock/${symbol}`)
  }

  useEffect(() => repositoryHub.recentSearch.subscribe(setRecent), [])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab') {
        const focusable = [mobileInputRef.current, closeRef.current].filter((item): item is HTMLInputElement | HTMLButtonElement => item !== null)
...
function ResultPanel({ query, results, activeIndex, onHover, onSelect }: ResultProps & { query: string }) { return <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0d1317] shadow-2xl"><div className="border-b border-white/[0.06] px-4 py-2.5 text-xs text-slate-500">{query ? `找到 ${results.length} 筆結果` : '最近搜尋'}</div><div className="max-h-[420px] overflow-y-auto p-1.5"><ResultRows results={results} activeIndex={activeIndex} onHover={onHover} onSelect={onSelect}/></div></div> }
