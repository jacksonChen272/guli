...
export function HealthScoreGauge({ health }: { health: StockHealthResult }) {
  ...
  return <div className="relative mx-auto w-full max-w-[300px]" role="img" aria-label={`健康總分 ${health.totalScore}，${health.grade}`}>
    ...
    <div className="absolute inset-x-0 bottom-0 text-center"><p className="mono text-4xl font-semibold text-white">{health.totalScore}</p><p className="mt-1 text-xs font-medium" style={{ color }}>{health.grade}</p><p className="mt-1 text-[9px] text-slate-600">滿分 100</p></div>
  </div>
}
