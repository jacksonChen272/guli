...
export function IndustryRotationPreview({ snapshot, loading }: { snapshot: IndustrySnapshot | null; loading: boolean }) {
  ...
  const industries = useMemo(() => [...(snapshot?.industries ?? [])].filter((item) => item.return1d !== null).sort((left, right) => (right.return1d ?? -Infinity) - (left.return1d ?? -Infinity)).slice(0, 5), [snapshot])
  return <section aria-labelledby="hot-industries-title" className="space-y-4">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">今日資金焦點</p><h2 id="hot-industries-title" className="type-section-title mt-2 font-semibold text-white">熱門族群</h2><p className="mt-2 text-sm text-slate-500">依實際產業樣本的平均漲跌排序，並揭露成交金額與市場廣度。</p></div><Button variant="ghost" onClick={() => navigate('/industries')} icon={<ArrowRight size={16}/>}>查看產業分析</Button></header>
    ...
    <p className="text-xs leading-5 text-slate-600">產業分類與強度目前含規則推導欄位，並非證交所完整官方產業分類；台股顏色採紅漲、綠跌。</p>
  </section>
}

function IndustryCard({ industry, index }: { industry: IndustrySnapshotItem; index: number }) {
  const navigate = useNavigate()
  const hasDerived = industry.sources.some((source) => source.type === 'derived' || source.type === 'mock')
  return <Card interactive className="min-w-0"><div className="p-5">
    <div className="flex items-center justify-between gap-2"><span className="mono text-xs text-slate-600">#{index + 1}</span><Badge tone={hasDerived ? 'warning' : 'info'}>{hasDerived ? '規則推導' : 'Official'}</Badge></div>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2"><Metric label="產業強度" value={industry.strengthScore.toFixed(0)}/><Metric label="平均漲跌" value={signed(industry.return1d)} tone={(industry.return1d ?? 0) >= 0 ? 'up' : 'down'}/><Metric label="成交金額" value={formatAmount(industry.tradingAmount)}/><Metric label="上漲／下跌" value={`${industry.advanceCount ?? '—'} / ${industry.declineCount ?? '—'}`}/></div>
