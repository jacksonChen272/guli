  ...
  return <>
    ...
    <aside className={panelClass}>
      ...
      <div className="border-t border-[var(--border-subtle)] p-3 pb-[max(12px,env(safe-area-inset-bottom))]">
        {!collapsed && <div className="mb-2 rounded-xl border border-brand-400/20 bg-brand-400/[.045] p-3.5"><p className="mono text-[13px] font-semibold text-brand-300">GULI v1.1.1</p><div className="mt-3 space-y-2 text-[13px] text-slate-300"><Status icon={<LayoutDashboard size={14}/>} text="TWSE Data Completion"/><Status icon={<Database size={14}/>} text="TWSE Official Data"/><Status icon={<GitBranch size={14}/>} text="Decision v1.0"/></div><p className="mono mt-3 border-t border-white/[.06] pt-2 text-[11px] text-slate-500">official · derived · mock 分層</p></div>}
