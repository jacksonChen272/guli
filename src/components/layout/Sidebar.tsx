  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex max-w-[86vw] flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-sidebar)] transition-[width,transform] duration-200 ${collapsed ? 'w-[272px] lg:w-20' : 'w-[272px]'} ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      ...
      <div className="border-t border-[var(--border-subtle)] p-3 pb-[max(12px,env(safe-area-inset-bottom))]">
        {!collapsed && <div className="mb-2 rounded-xl border border-brand-400/20 bg-brand-400/[.045] p-3.5"><p className="mono text-[13px] font-semibold text-brand-300">GULI v1.0.0-beta.3.1</p><div className="mt-3 space-y-2 text-[13px] text-slate-300"><Status icon={<Database size={14}/>} text="TWSE Official History"/><Status icon={<GitBranch size={14}/>} text="Decision Engine v1.0"/><Status icon={<Layers3 size={14}/>} text="Market Heatmap"/></div><p className="mono mt-3 border-t border-white/[.06] pt-2 text-[11px] text-slate-500">official · derived · mock 分層</p></div>}
