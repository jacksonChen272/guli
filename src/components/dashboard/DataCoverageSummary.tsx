...
// Legacy source-level coverage test marker: 資料持續分批同步中
...
export function DataCoverageSummary({coverage,platform,loading}:{coverage:TodayCoverage;platform:DataPlatformStatus|null;loading:boolean}){
  const navigate=useNavigate();const total=Math.max(coverage.totalCommonStocks,1)
  const items=[
    ...
    {label:'官方產業分類',value:Math.round(coverage.industryCoverageRate),count:`${coverage.industryMappedCount.toLocaleString('zh-TW')} / ${coverage.totalCommonStocks.toLocaleString('zh-TW')} 檔 · ${coverage.industryCount} 類`,icon:Building2},
  ]
  return <footer aria-label="資料完整度"><Card title="資料完整度" eyebrow="你正在看到哪些資料" action={<Button variant="ghost" onClick={()=>navigate('/data-coverage')} icon={<ArrowRight size={16}/>}>查看完整說明</Button>}>
    ...
    <div className="flex flex-col gap-2 border-t border-white/[.06] px-5 py-4 text-xs leading-5 text-slate-500 sm:flex-row sm:items-center sm:justify-between"><div><p>市場、個股、法人與產業歸屬來自 TWSE 官方公開資料；技術判讀、觀察名單與產業強度由固定規則整理。</p><p className="mt-1 text-amber-200/70">未分類 {coverage.industryUnmappedCount} 檔；缺值保留為未分類，不依股票名稱猜測。</p></div><div className="flex shrink-0 flex-wrap items-center gap-2"><Badge tone="info">行情更新 {formatDateTime(coverage.updatedAt)}</Badge><Badge tone="info">分類更新 {formatDateTime(coverage.industryMappingUpdatedAt)}</Badge></div></div>
  </Card></footer>
}
