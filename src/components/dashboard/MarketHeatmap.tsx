...
export function MarketHeatmap({ dataset, loading, error }: { dataset: MarketHeatmapDataset | null; loading: boolean; error?: string | null }) {
  ...
  const option = useMemo<EChartsOption>(() => ({
    animationDuration: 220, animationDurationUpdate: 180,
    ...
    series: [{ type: 'treemap', roam: false, nodeClick: false, breadcrumb: { show: false }, squareRatio: 1.15, visibleMin: 12,
      ...
      itemStyle: { borderColor: '#0a1114', borderWidth: 2, gapWidth: 2 }, emphasis: { label: { color: '#fff', fontWeight: 'bold' }, itemStyle: { borderColor: 'rgba(255,255,255,.55)', borderWidth: 2 } },
  ...
  const events = useMemo(() => ({ click: (raw: unknown) => { const node = (raw as { data?: ChartDatum }).data?.node; if (!node) return; if (node.type === 'industry' && node.industryId) setIndustryId(node.industryId); if (node.type === 'stock' && node.symbol) navigate(`/stock/${node.symbol}`) } }), [navigate])
  ...
  return <section aria-labelledby="market-heatmap-title" className="space-y-4">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="eyebrow">盤後市場分布</p><h2 id="market-heatmap-title" className="type-section-title mt-2 font-semibold text-white">市場熱力圖</h2><p className="mt-2 text-sm leading-6 text-slate-500">依成交金額呈現市場關注度，顏色代表當日漲跌；產業優先採 TWSE 官方分類。</p></div>
      <div className="flex flex-wrap gap-2"><Badge tone={sourceTone}>{summary?.sourceLabel ?? 'Missing'}</Badge>{dataset && <Badge tone={(dataset.mappingStatus ?? 'Mixed') === 'Official' ? 'info' : 'warning'}>{dataset.mappingStatus ?? 'Mixed'} Mapping</Badge>}{dataset?.status === 'stale' && <Badge tone="warning">Stale</Badge>}</div>
    </header>
    <Card className="min-w-0 overflow-hidden">
      ...
      {!dataset || !visualNodes.length ? <MarketHeatmapEmptyState loading={loading} error={error}/> : <div ref={containerRef} className="min-w-0">
        <div className="flex flex-col gap-3 border-b border-white/[.05] px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5"><MarketHeatmapLegend metric={colorMetric}/><span className="shrink-0 text-xs text-slate-600">{mode === 'stock' || industryId ? `顯示前 ${visualNodes.length} 檔成交金額樣本` : `${visualNodes.length} 個產業群組`}</span></div>
        <div role="img" aria-label={`${industry?.name ?? (mode === 'industry' ? '產業' : '個股')}市場熱力圖`} className="min-w-0 overflow-hidden"><ReactECharts ref={chartRef} echarts={echarts} option={option} notMerge lazyUpdate onEvents={events} style={{ height: 'clamp(340px, 48vw, 520px)', width: '100%' }} opts={{ renderer: 'canvas' }}/></div>
      </div>}
      {dataset && <div className="grid gap-3 border-t border-white/[.06] p-4 text-xs text-slate-500 sm:grid-cols-2 xl:grid-cols-4 sm:p-5">
        <p className="flex items-start gap-2"><Database className="mt-0.5 shrink-0" size={14}/>普通股 {dataset.totalStockUniverse} 檔；官方分類 {dataset.officialMappedStockCount ?? dataset.mappedStockCount} 檔。</p>
        <p className="flex items-start gap-2"><Layers3 className="mt-0.5 shrink-0" size={14}/>官方覆蓋 {(dataset.officialCoverageRate ?? dataset.coverageRate).toFixed(2)}%；衍生群組 {dataset.derivedMappedStockCount ?? 0} 檔（{(dataset.derivedCoverageRate ?? 0).toFixed(2)}%）。</p>
        <p className="flex items-start gap-2"><Info className="mt-0.5 shrink-0" size={14}/>未分類 {dataset.unmappedStockCount} 檔；不依股票名稱猜測產業。</p>
        <p className="flex items-start gap-2"><CalendarDays className="mt-0.5 shrink-0" size={14}/>交易日 {dataset.tradeDate}；分類更新 {dataset.mappingEffectiveDate ?? '尚未取得'}。</p>
      </div>}
    </Card>
  </section>
}
