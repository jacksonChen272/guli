...
const lineData = (points: Array<{ tradeDate: string; value: number | null }>): LineData<Time>[] =>
  points.flatMap((point) =>
    point.value === null ? [] : [{ time: point.tradeDate as Time, value: point.value }])

const lineMap = (points: Array<{ tradeDate: string; value: number | null }>) =>
  new Map(points.map((point) => [point.tradeDate, point.value]))

export function CandlestickPriceChart({
  series,
  visibility,
  zones = [],
  onMountedChange,
  ...
  const tooltipIndex = useMemo(() => ({
    price: new Map(series.prices.map((point) => [point.tradeDate, point])),
    ma5: lineMap(series.ma5),
    ma20: lineMap(series.ma20),
    ma60: lineMap(series.ma60),
    ma120: lineMap(series.ma120),
  }), [series])
  ...
  useEffect(() => {
    ...
    void import('lightweight-charts').then(({
      ...
      chart = createChart(container, {
        ...
        layout: {
          background: { type: ColorType.Solid, color: '#0b111d' },
          textColor: '#94a3b8',
          attributionLogo: true,
        },
        grid: {
          vertLines: { color: 'rgba(148,163,184,.04)' },
          horzLines: { color: 'rgba(148,163,184,.04)' },
        },
        crosshair: { mode: CrosshairMode.Normal },
        rightPriceScale: {
          borderColor: 'rgba(148,163,184,.12)',
          scaleMargins: { top: 0.08, bottom: visibility.volume ? 0.24 : 0.08 },
        },
        timeScale: {
          borderColor: 'rgba(148,163,184,.12)',
          timeVisible: true,
      ...
      candle.setData(series.prices.flatMap((point): CandlestickData<Time>[] =>
        [point.open, point.high, point.low, point.close].some((value) => value === null)
          ? []
          : [{
      ...
      if (visibility.zones) {
        zones.forEach((zone) => {
          const color = zone.type === 'support' ? 'rgba(47,197,154,.55)' : 'rgba(239,106,120,.55)'
      ...
      crosshairHandler = (parameter) => {
        ...
        const date = String(parameter.time)
        const value = parameter.seriesData.get(candle)
        const source = tooltipIndex.price.get(date)
        if (value && 'open' in value) {
          setTooltip({
            ...
            ma5: tooltipIndex.ma5.get(date) ?? null,
            ma20: tooltipIndex.ma20.get(date) ?? null,
            ma60: tooltipIndex.ma60.get(date) ?? null,
            ma120: tooltipIndex.ma120.get(date) ?? null,
          })
        }
      }
  ...
  const show = (value: number | null) =>
    value === null ? '—' : value.toLocaleString('zh-TW', { maximumFractionDigits: 2 })

  return (
    <LightweightChartContainer
      ref={containerRef}
      ariaLabel="TWSE 官方歷史行情 K 線、成交量、均線與支撐壓力區"
    >
      {!ready && (
        <div
          className="absolute inset-0 animate-pulse bg-slate-900/70 motion-reduce:animate-none"
          aria-label="圖表載入中"
        />
      )}
      {tooltip && (
        <div
          data-testid="stock-chart-tooltip"
          className="pointer-events-none absolute left-3 top-3 z-10 max-w-[min(320px,calc(100%-24px))] break-words rounded-lg border border-white/10 bg-slate-950/95 px-3 py-2 text-[11px] leading-5 text-slate-300 shadow-xl [overflow-wrap:anywhere]"
          role="status"
        >
          <p className="font-medium text-white">{tooltip.date}</p>
          <p className="mt-1">開 {show(tooltip.open)}　高 {show(tooltip.high)}　低 {show(tooltip.low)}　收 {show(tooltip.close)}</p>
          <p className="mt-1 text-slate-500">
            量 {tooltip.volume?.toLocaleString('zh-TW') ?? '—'} · MA5 {show(tooltip.ma5)} · MA20 {show(tooltip.ma20)}
            {' · '}MA60 {show(tooltip.ma60)} · MA120 {show(tooltip.ma120)}
          </p>
        </div>
      )}
    </LightweightChartContainer>
