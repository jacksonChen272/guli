...
describe('Full-market foundation and GitHub Pages guard', () => {
  ...
  it('uses validated atomic writes with transient Windows lock retry', async () => { const source = await text('scripts/data/history/HistoryManifestWriter.ts'); expect(source).toContain("JSON.parse(await readFile(temporary, 'utf8'))"); expect(source).toContain('renameWithTransientLockRetry(temporary, file)'); expect(source).toContain("code !== 'EPERM'") })
  ...
  it('Sidebar displays v1.2.0', async () => expect(await text('src/components/layout/Sidebar.tsx')).toContain('GULI v1.2.0'))
